  ...
  const TRUST_ITEMS = [
    {
      ...
      label: <span dir="ltr">{`${SITE.address.city}, ${SITE.address.region} ${SITE.address.postalCode}`}</span>,
      sublabel: t('verifyStrip.homeBaseSublabel'),
    },
    {
      key: 'established',
      icon: (
      ...
      label: t('verifyStrip.establishedLabel'),
      sublabel: t('verifyStrip.establishedSublabel'),
    },
    {
      key: 'phone',
      icon: (
      ...
      label: <span dir="ltr">{SITE.phone.display}</span>,
      sublabel: t('verifyStrip.verifiedBusinessSublabel'),
    ...
    },
    {
      ...
      label: t('verifyStrip.ownersLabel'),
      sublabel: t('verifyStrip.ownersSublabel'),
    },
    {
      key: 'licensed',
      icon: (
      ...
      label: t('verifyStrip.licensedLabel'),
      sublabel: t('verifyStrip.licensedSublabel'),
    },
  ]

  return (
    <section className="bg-charcoal-800 border-y border-charcoal-600" aria-label={t('verifyStrip.ariaLabel')}>
      <FadeIn>
        <div className="container-max section-padding py-8">
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 sm:gap-6">
            {TRUST_ITEMS.map((item) => {
              ...
              return item.href ? (
                <a key={item.key} href={item.href} className="contents">
                  {content}
                </a>
              ) : (
