...
- Form library: `react-hook-form` (already in deps) vs plain `useState`. Picked **plain `useState`** — both forms are short, server-side Zod is the validation source of truth, and dropping RHF kept the bundle tighter. RHF removed from deps.
- Form-error UX: inline-only vs `aria-live` summary region + inline. Picked **both** — server returns `{ error, fields }`, the summary region is `role="alert" aria-live="polite"`, and per-field messages get `aria-invalid` + `aria-describedby` for screen readers.
